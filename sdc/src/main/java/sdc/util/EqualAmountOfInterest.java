 ...
 * 等额本息
 */
public class EqualAmountOfInterest {
    //todo:根据还款日期，计算利息
     ...
     */
    public static List<EqualAmountBean> execEqualAmountOfInterest(double quota, double interestRate, double interestRebate, Date dateOfLoan, int stageNumberOfMonth) {
        ...
        for (int i = 1; i <= stageNumberOfMonth; i++) {
            ...
            //折扣
            if (interestRebate > 0) {
                _interestRate = _quota.multiply(BigDecimal.valueOf(days)).multiply(BigDecimal.valueOf(interestRate))
